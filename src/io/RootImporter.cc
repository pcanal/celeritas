...
//---------------------------------------------------------------------------//
/*!
 * Load GdmlGeometryMap from the ROOT file to populate MaterialParams
 */
std::shared_ptr<MaterialParams> RootImporter::load_material_data()
{
    ...
    auto elements_map = geometry.elemid_to_element_map();
    for (const auto& elem_key : elements_map)
    {
    ...
    auto materials_map = geometry.matid_to_material_map();
    for (const auto& mat_key : materials_map)
    {
        MaterialParams::MaterialInput material_params;
        material_params.name           = mat_key.second.name;
        material_params.number_density = mat_key.second.atomic_density;
        ...
        for (const auto& elem_key : mat_key.second.elements_fractions)
        {
            ElementDefId elem_def_id{elem_key.first};
            real_type    fraction = elem_key.second;
            material_params.elements.push_back({elem_def_id, fraction});
        }
