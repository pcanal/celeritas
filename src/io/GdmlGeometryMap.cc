 ...
 */
size_type GdmlGeometryMap::sizeof_largest_material_element_list() const
{
    size_type result = 0;
    for (const auto& key : matid_to_material_)
    {
        if (result < key.second.elements_fractions.size())
            result = key.second.elements_fractions.size();
    }
    return result;
}
